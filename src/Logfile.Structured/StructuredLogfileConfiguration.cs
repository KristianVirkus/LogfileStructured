...
namespace Logfile.Structured
{
    ...
    {
        internal const string DateTimeFormat = "yyyyMMdd-HHmmssfff";
        internal const string DateTimeFormatRegexText = "[0-9]{8}\\-[0-9]{9}";
        ...
        /// <summary>
        /// Creates a regular expression covering the configured
        /// file name format. The created expression may not match
        /// exactly.
        /// </summary>
        /// <returns>The regular expression.</returns>
        public Regex BuildRegexFromFileNameFormat()
            => new Regex(this.FileNameFormat
                .Replace("{app-name}", $"({Regex.Escape(this.AppName)})")
                .Replace("{start-up-time}", DateTimeFormatRegexText)
                .Replace("{creation-time}", DateTimeFormatRegexText)
                .Replace("{seq-no}", "[0-9]+"));
        ...
        /// </summary>
        /// <param name="fileSequenceNo">The file sequence number.</param>
        /// <returns>The final filename.</returns>
        public string BuildFileName(int fileSequenceNo)
            => this.FileNameFormat
                .Replace("{app-name}", this.AppName)
                .Replace("{start-up-time}", System.Diagnostics.Process.GetCurrentProcess().StartTime.ToString(DateTimeFormat))
                .Replace("{creation-time}", DateTime.Now.ToString(DateTimeFormat))
                .Replace("{seq-no}", fileSequenceNo.ToString());
    }
}
